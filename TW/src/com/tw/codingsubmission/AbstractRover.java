...
public abstract class AbstractRover implements IRover {
	...
	protected AbstractRover(GridCoordinates coord, Orientation orientation, Grid grid){
		if (null == coord){
			throw new IllegalArgumentException("Cannot Construct Rover with null Grid Position");
		}
		if (null == orientation){
			throw new IllegalArgumentException("Cannot Construct Rover with null Orientation");
		}
		if (null == grid){
			throw new IllegalArgumentException("Cannot Construct Rover with null Grid");
		}
		
		if (!grid.isValidCoordinate(coord.getX(), coord.getY())){
			throw new IllegalArgumentException("Coordinates " + coord.toString() + " not valid for provided Grid.");
		}
		_gridPosition = coord;
	...
	public void move() {
		...
		switch (_orientation.getCurrentOrientation()){
			...
			case WEST:
				...
				break;
			default:
				throw new IllegalStateException("bad orientation for move: " + _orientation.getCurrentOrientation());
		}
		
		if (_grid.isValidCoordinate(targetX, targetY)){
			_gridPosition.set(targetX, targetY);
		}else{
			System.out.println("Coordinate: (" + targetX + "," + targetY + ") is not a valid grid coordinate. Not moving Rover.");
		}
		
	}

	public String getPositionAndHeading() {
		return getX() + " " + getY() + " " + getOrientation().toString().charAt(0);
	}
	...
	public void executeCommands () {
		synchronized(_queueMutex){				//right now we make push and exec mutually exclusive...ok for now but could possibly be a problem if there is a long running command
