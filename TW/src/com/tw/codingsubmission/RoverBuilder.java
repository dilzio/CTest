	...
	public IRover build(Grid grid, RoverSpec spec) {
		try {
			...
			case 'W':
				...
				break;
			default:
				throw new RuntimeException("Unknown Orientation for value: "
						+ orientationDirection);
			}
			GridCoordinates startingCoordinates = new GridCoordinates(spec.x(),
					spec.y());
			...
			for (int i = 0; i < spec.commands().length(); i++) {
				...
				case ('M'):
					...
					break;
				default:
					throw new RuntimeException("Unknown Command Code: "
							+ commandCode);
				}
			}
			return rover;
